...
exports.ChangeTraffic = function(traffic_id, type, company, money, start_position,end_positon, start_time,end_time, callback) {
    var daaF = getDateByTimeStr(start_time);
    var daaE = getDateByTimeStr(end_time);
    console.log(traffic_id+" " +type+" " +company+ " " +money+ " " +start_position+ " " +end_positon+ " " +daaF+ " " +daaE+"ChangeNew=ChangeNew=CzhangeNew");
    var updateSql = 'UPDATE t_traffic SET `type`=?,`company`=?, money=?, start_position=? ,end_position=?, start_time=? ,end_time=? WHERE traffic_id=?';
    // var updateSql = 'UPDATE t_traffic SET type=? WHERE traffic_id=?';
    console.log(updateSql);
    var updateParams = [type, company, money, start_position,end_positon, daaF, daaE,traffic_id];
